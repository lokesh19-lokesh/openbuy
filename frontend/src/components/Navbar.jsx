...
import { ShoppingCart, LogOut, User, Shield, Home, Search, FileText, LayoutDashboard, Package, ClipboardList } from 'lucide-react';
...
const Navbar = () => {
  ...
  return (
              ...
              <div className="flex items-center space-x-1 sm:space-x-3">
                ...
                {user.profile?.role === 'seller' && (
                  <div className="hidden sm:flex items-center space-x-1 sm:space-x-3">
                    ...
                    <Link to="/seller/inventory" className="px-3 py-2 hover:bg-gray-100 rounded-full transition-colors group flex items-center" title="Inventory">
                      <span className="text-sm font-bold text-gray-600 group-hover:text-black">Inventory</span>
                    </Link>
                    <Link to="/seller/orders" className="px-3 py-2 hover:bg-gray-100 rounded-full transition-colors group flex items-center" title="Orders">
                      <span className="text-sm font-bold text-gray-600 group-hover:text-black">Orders</span>
                    </Link>
                    <Link to="/seller/profile" className="px-3 py-2 hover:bg-gray-100 rounded-full transition-colors group flex items-center" title="Profile">
                ...
                {/* Mobile Top Profile Dropdown (Buyers & Sellers) */}
                {user.profile?.role !== 'admin' && (
                  <div className="sm:hidden relative">
                    ...
                    {profileOpen && (
                      <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50">
                        ...
                        <Link to={user.profile?.role === 'seller' ? "/seller/profile" : "/buyer/profile"} onClick={() => setProfileOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 font-semibold">My Profile</Link>
    ...
    {/* Mobile Bottom Navigation for Sellers */}
    {user && user.profile?.role === 'seller' && (
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-2 py-2 flex justify-between items-center z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <NavLink to="/" end className={({ isActive }) => `flex flex-col items-center justify-center w-full py-2 rounded-xl transition-colors ${isActive ? 'text-blue-700 bg-blue-50' : 'text-gray-400'}`}>
          {({ isActive }) => (
            <>
              <Home size={22} className="mb-1" strokeWidth={isActive ? 2.5 : 2} />
              <span className="text-[10px] font-bold uppercase tracking-wide">Home</span>
        ...
        <NavLink to="/seller" end className={({ isActive }) => `flex flex-col items-center justify-center w-full py-2 rounded-xl transition-colors ${isActive ? 'text-blue-700 bg-blue-50' : 'text-gray-400'}`}>
          {({ isActive }) => (
            <>
        ...
        <NavLink to="/seller/inventory" className={({ isActive }) => `flex flex-col items-center justify-center w-full py-2 rounded-xl transition-colors ${isActive ? 'text-blue-700 bg-blue-50' : 'text-gray-400'}`}>
          {({ isActive }) => (
            <>
        ...
        <NavLink to="/seller/orders" className={({ isActive }) => `flex flex-col items-center justify-center w-full py-2 rounded-xl transition-colors ${isActive ? 'text-blue-700 bg-blue-50' : 'text-gray-400'}`}>
          {({ isActive }) => (
            <>
              <ClipboardList size={22} className="mb-1" strokeWidth={isActive ? 2.5 : 2} />
              <span className="text-[10px] font-bold uppercase tracking-wide">Orders</span>
            </>
          )}
        </NavLink>
